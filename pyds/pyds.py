...
class MassFunction(dict):
        ...
        if sample_count == None:  # deterministic
            ...
            def traverse(m, likelihoods, ones, index, hyp, mass):
                if index == len(likelihoods):
                    ...
                    traverse(
                        m, likelihoods, ones, index + 1, hyp + [likelihoods[index][0]], mass * likelihoods[index][1]
                    )
    ...
    def from_pl(pl):
        ...
        bel_theta = pl[frame]
        bel = {
            frozenset(frame - h): bel_theta - v for (h, v) in pl.items()
    ...
    def __str__(self):
        ...
        return "{" + "; ".join([str(set(h)) + ":" + f"{v:.2}" for (v, h) in hyp]) + "}"
    ...
    def _combine(self, mass_function, rule, normalization, sample_count, importance_sampling):
        ...
        for m in mass_function:
            if not isinstance(m, MassFunction):
                raise TypeError(
                    "expected type MassFunction but got %s; make sure to use keyword arguments for anything other than mass functions"
                    % type(m)
                )
    ...
    def from_possibility(poss):
        ...
        H, P = zip(
            *sorted(poss.items(), key=lambda e: e[1], reverse=True)
    ...
    def _confidence_intervals(histogram, alpha):
        ...
        p_lower = {}
        p_upper = {}
        a = chi2.ppf(1.0 - alpha / len(histogram), 1)
        n = float(sum(histogram.values()))
        for h, n_h in histogram.items():
            delta_h = a * (a + 4.0 * n_h * (n - n_h) / n)
            p_lower[h] = (a + 2.0 * n_h - sqrt(delta_h)) / (2.0 * (n + a))
            p_upper[h] = (a + 2.0 * n_h + sqrt(delta_h)) / (2.0 * (n + a))
            ...
            for i1, h1 in enumerate(H):
                ...
                for i2, h2 in enumerate(H[i1 + 1 :]):
                    ...
                    if i2 == i1 + 1:
                        ...
                        v = (
                            p_lower_set(H[i1 : i2 + 1])
                            - p_lower_set(H[i1 + 1 : i2 + 1])
                            - p_lower_set(H[i1:i2])
                            + p_lower_set(H[i1 + 1 : i2])
                        )
                    if v > 0:
                        m[H[i1 : i2 + 1]] = v
            return m
        ...
        if approximate:
            ...
            for k, h_k in enumerate(H):
                ...
                for A in powerset(P_k):
                    ...
                    # constraint (27)
                    cons.append(lambda p: 1.0 - sum(p))
                    cons.append(lambda p: sum(p) - 1.0)
                    # constraint (30)
                    for i in G:
...
def gbt_pignistic(singleton, likelihoods):
    ...
    return sum(eta * v * singleton_lh / (c + 1.0) for c, v in enumerate(m_sum))
